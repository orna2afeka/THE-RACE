...
export default function Sidebar({
  live, config, hidden, manualLap, setManualLap, rivalLaps, setRivalLaps,
  fontScale, setFontScale, serverNowS,
}: {
  live: Live | null; config: Config; hidden: boolean;
  manualLap: number; setManualLap: (n: number) => void;
  rivalLaps: number; setRivalLaps: (n: number) => void;
    ...
    <aside className="sidebar" hidden={hidden}>
      <Sec icon="radio" title="Race status">
        <div className="kv">
          ...
          <KV k="Gap (laps)" v={fmt(live?.gapToCompetitor ?? null, '+.0f')} mono />
      ...
      <Sec icon="flag" title="Race control">
        ...
        <label className="fld">Rival lap count</label>
        <input type="number" value={rivalLaps} onChange={(e) => setRivalLaps(Number(e.target.value))} />
      </Sec>

      <Sec icon="timer" title="Driver stint">
        <StintPanel stint={live?.driverStint} serverNowS={serverNowS}
...
function ExportPanel({ config }: { config: Config }) {
  ...
  if (!data || !data.total || data.lo === null) {
    return <Sec icon="download" title="Export"><div className="caption">No telemetry stored yet — start collector.py.</div></Sec>;
  }
  const a = from || localInput(data.lo);
  const b = to || localInput(data.hi ?? data.lo);
  const url = `/api/export/telemetry.xlsx?start=${new Date(a).getTime() / 1000}`
    + `&end=${new Date(b).getTime() / 1000}&groups=${encodeURIComponent(groups.join(','))}`;
    ...
    <Sec icon="download" title="Export (Excel)">
      ...
      <div className="chips">
        {config.exportGroups.map((g) => (
          <button key={g} className="chip" aria-pressed={groups.includes(g)}
                  onClick={() => setGroups((s) => s.includes(g) ? s.filter((x) => x !== g) : [...s, g])}>{g}</button>
        ))}
      </div>
      <label className="fld">From</label>
      <input type="datetime-local" value={a} onChange={(e) => setFrom(e.target.value)} />
      <label className="fld">To</label>
      <input type="datetime-local" value={b} onChange={(e) => setTo(e.target.value)} />
      <a className="btn primary block" href={groups.length ? url : undefined}
         style={{ marginTop: 10, opacity: groups.length ? 1 : 0.45 }}
         onClick={() => groups.length && toast('Building the workbook…', 'info')}>
