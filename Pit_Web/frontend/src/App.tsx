...
import { MISSING, ageText, fmt, hms, lapTime, useConfig, useLive, useStored } from './lib';
...
const STRIP_TREND = ['Speed', 'MotorTemp', 'CtrlTemp', 'SoC', 'BattTemp', 'Power', 'Energy'];
  ...
  return (
    <>
      ...
      <div className="grid g7">
        ...
        <MetricTile title="Battery temp" value={fmt(s.batt_temp)} unit="°C" tier={t.battTemp} trend={trend.BattTemp} />
...
export default function App() {
  ...
  const [rivalLaps, setRivalLaps] = useStored('pit.rivalLaps', 0);
  const [sidebarOpen, setSidebarOpen] = useStored('pit.sidebar', true);
  const { live, connected, silentS, clockOffsetMs } = useLive(manualLap, rivalLaps);
  const link = linkState(live, connected, silentS);

  // A 1 s tick for the race clock, against the SERVER's clock. The socket only
  // pushes every 2 s and a clock that jumps in twos reads as broken.
  const [now, setNow] = useState(Date.now());
  useEffect(() => { const id = window.setInterval(() => setNow(Date.now()), 1000); return () => clearInterval(id); }, []);
  const serverNowS = (now + clockOffsetMs) / 1000;
  ...
  const remaining = race?.isRacing && race.startTime
    ? Math.max(0, 24 * 3600 - (serverNowS - race.startTime))
    : 24 * 3600;
    ...
    <div className="shell">
      <header className="bar">
        ...
        <div className="bar-center">
          ...
          <StintClock stint={live?.driverStint} serverNowS={serverNowS} />
        </div>
      ...
      <div className="body">
        <Sidebar live={live} config={config} hidden={!sidebarOpen}
                 ...
                 rivalLaps={rivalLaps} setRivalLaps={setRivalLaps}
                 fontScale={fontScale} setFontScale={setFontScale}
                 serverNowS={serverNowS} />
        <main className="main">
          {frozenSince && (
          ...
          <StintBanner stint={live?.driverStint} serverNowS={serverNowS} />
          ...
          <div hidden={tab !== 'History'}>
            <ErrorBoundary name="History"><History config={config} dark={dark} visible={tab === 'History'} /></ErrorBoundary>
          </div>
