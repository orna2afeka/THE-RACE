...
_STATE_COLUMNS = {
    ...
    "batt_temp": "battery_temp_C", "rpm": "mms_rpm",
...
#   solar_sensor_status explains WHY solar_current is missing right now (night,
#     cloud, unplugged); a stale reason beside a live reading would mislead.
#   lap_source is only meaningful paired with the lap that just happened.
_NO_CARRY_FORWARD = {"solar_status", "lap_source"}
...
def read_live_state(conn):
    ...
    state.update({
        "motor_map": None, "solar_status": None, "throttle_zone": None,
...
def build_live(conn, manual_lap=-1, rival_laps=0):
    ...
    return {
        ...
        "gapToCompetitor": None if active_lap is None else active_lap - rival_laps,
        "odometerKm": odo_km,
...
@app.get("/api/live")
def api_live(manual_lap: int = Query(-1), rival_laps: int = Query(0)):
    with closing(ro_conn()) as conn:
        return build_live(conn, manual_lap, rival_laps)
...
def api_export_xlsx(start: float | None = Query(None),
    ...
    data, n = export.to_xlsx_bytes(start_ts=start, end_ts=end, metrics=cols)
...
async def ws_live(ws: WebSocket):
    """Pushes the whole fast tier every 2 s: tiles, fault banner, sectors, map.
    ...
    try:
        while True:
            ...
            try:
                msg = await asyncio.wait_for(ws.receive_json(), timeout=FAST_TICK_S)
                manual_lap = int(msg.get("manualLap", manual_lap))
                rival_laps = int(msg.get("rivalLaps", rival_laps))
            except asyncio.TimeoutError:
                pass
