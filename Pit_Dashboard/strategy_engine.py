...
# Example charging curve - replace with real battery data


def get_charging_power(soc):
    if soc <= 5:
    ...
    lower_soc = int(soc // 5) * 5
    upper_soc = lower_soc + 5

    lower_power = CHARGING_CURVE[lower_soc]
    upper_power = CHARGING_CURVE[upper_soc]

    power = lower_power + (
        (soc - lower_soc) / (upper_soc - lower_soc)
    ) * (upper_power - lower_power)

    return power


def calculate_charging_time(start_soc, target_soc, battery_capacity_kwh):
    ...
    while soc < target_soc:
        next_soc = min(soc + soc_step, target_soc)

        middle_soc = (soc + next_soc) / 2
        power_kw = get_charging_power(middle_soc)
        ...
        total_time_min += (energy_added_kwh / power_kw) * 60
        soc = next_soc
...
# ==============================================================================
# SOC-DEPENDENT CHARGING STRATEGY
# ==============================================================================

def calculate_soc_charging_strategy(
    time_left_min,
    current_available_wh,
    current_lap,
    consumption_table,
